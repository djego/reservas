      ...
      <div style="float:left; width: 720px;">
        ...
        foreach ($lst_ciudad as $val) { $i++;
          if ($ciudad != $val["city_id"]) {
            $ciudad = $val["city_id"];
            if ($val["nr_hotels"] < sfConfig::get('app_nro_hoteles')) {
              if ($i < sfConfig::get('app_nro_max_cities')) {
                $name = utf8_decode($val["name"]);
                $slug_city = Utils::slugify($val["name"]);?>
                <div class="one-fourth pt20">
                  <h3>Hotel en <?php echo $name ?></h3>
                  <div class="image"><img src="<?php echo sfConfig::get('app_s_img'); ?>ciudad/ad_<?php echo $ciudad ?>.jpg" alt="" /></div>
                  <div class="text-button"><a href="<?php echo url_for('city_hotels',array('id' => $ciudad,'slug' => $slug_city)) ?>">Ver más</a> <img src="<?php echo sfConfig::get('app_s_img'); ?>arrow.jpg" width="6" height="7" alt="" class="vm"/></div>
                </div>
        <?php
              }
            }
